...
// Global variable to control whether weights should be set to 1
bool set_weights_to_one = true; // Default value is false
...
void compute_weights(const map<string, pair<float, double>>& atom_map, float max_prop, float min_prop, float alpha) 
{
    for (const auto& entry : atom_map) {
        ...
        if (set_weights_to_one == 1){
            weight = 1;
        } else{
            weight = alpha + (1 - alpha) * ((prop - min_prop) / (max_prop - min_prop));
        }
...
double get_dist(xyz box, xyz a1, xyz a2, map<string, pair<float, double>> atom_map)
{
    ...
    string atom_pair = (atom_type1 < atom_type2) ? (atom_type1 + atom_type2) : (atom_type2 + atom_type1);
...
void writeClusterDataComp(const string& filename, const vector<string>& data) 
{
    ofstream ofs(filename);
    if (!ofs) {
        cerr << "Error: Could not open file " << filename << endl;
    ...
    for (const auto& row : data) {
        buffer << row << "\n";  // Each string in 'data' is written as a separate line
    }
...
int main(int argc, char* argv[]) {
    ...
    vector<string> clutypes_2b(1);
    vector<string> clutypes_3b(3);
    vector<string> clutypes_4b(6);
...
for (int i = 0; i < natoms; i++)
{
    for (int j = i + 1; j < natoms; j++)
    {
        ...
        if (unweighted_dist_ij < rcout_2b)
        {
            ...
            clutypes_2b.push_back(coords[i].atom_type + " " + coords[j].atom_type);
            ...
            for (int k = j + 1; k < natoms; k++)
            {
                ...
                clutypes_3b.push_back(coords[i].atom_type + " " + coords[j].atom_type + " " + coords[k].atom_type);
                ...
                for (int l = k + 1; l < natoms; l++)
                {
                    ...
                    clutypes_4b.push_back(coords[i].atom_type + " " + coords[j].atom_type + " " + coords[k].atom_type + " " + coords[l].atom_type);
                }
            }
        }
    }
    ...
    writeClusterDataComp("clutypes_2b.txt", clutypes_2b);
    writeClusterDataComp("clutypes_3b.txt", clutypes_3b);
    writeClusterDataComp("clutypes_4b.txt", clutypes_4b);
