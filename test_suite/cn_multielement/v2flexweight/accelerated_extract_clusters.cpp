...
struct ThreadData {
    ...
    double rcout_2b, rcout_3b, rcout_4b;
    ...
    // Default constructor
    ThreadData() : start(0), end(0), coords(coords), boxdims(boxdims), atom_map(atom_map),
                   rcout_2b(0), rcout_3b(0), rcout_4b(0),
                   cludists_2b_direct(cludists_2b_direct),
                   cludists_3b_direct(cludists_3b_direct),
                   cludists_4b_direct(cludists_4b_direct),
                   cludists_2b_trans(cludists_2b_trans),
                   cludists_3b_trans(cludists_3b_trans),
                   cludists_4b_trans(cludists_4b_trans) {}
};
...
// Function to compute distances and transformations
void compute_clusters(int start, int end, const vector<xyz>& coords, const xyz& boxdims, const map<string, pair<float, double>>& atom_map,
                      double rcout_2b, double rcout_3b, double rcout_4b, vector<vector<double>>& cludists_2b_direct,
                      vector<vector<double>>& cludists_3b_direct, vector<vector<double>>& cludists_4b_direct,
                      vector<vector<double>>& cludists_2b_trans, vector<vector<double>>& cludists_3b_trans,
                      vector<vector<double>>& cludists_4b_trans) {
    vector<double> dist_2b(1), dist_3b(3), dist_4b(6);
    vector<double> dist_2b_trans(1), dist_3b_trans(3), dist_4b_trans(6);
    ...
    for (int i = start; i < end; i++) {
        for (int j = i + 1; j < coords.size(); j++) {
            ...
            if (unweighted_dist_ij < rcout_2b) {
                ...
                if (unweighted_dist_ij >= rcout_3b) continue;
                ...
                for (int k = j + 1; k < coords.size(); k++) {
                    ...
                    if (unweighted_dist_ik >= rcout_3b || unweighted_dist_jk >= rcout_3b) continue;
                    ...
                    if (unweighted_dist_ij >= rcout_4b || unweighted_dist_ik >= rcout_4b || unweighted_dist_jk >= rcout_4b) continue;
                    ...
                    for (int l = k + 1; l < coords.size(); l++) {
                        ...
                        if (unweighted_dist_il >= rcout_4b || unweighted_dist_jl >= rcout_4b || unweighted_dist_kl >= rcout_4b) continue;
...
void* thread_function(void* arg) {
    ...
    compute_clusters(data->start, data->end, data->coords, data->boxdims, data->atom_map, data->rcout_2b, data->rcout_3b, data->rcout_4b,
                     data->cludists_2b_direct, data->cludists_3b_direct, data->cludists_4b_direct,
                     data->cludists_2b_trans, data->cludists_3b_trans, data->cludists_4b_trans);
    return nullptr;
}
...
const int num_threads = 4; // Adjust based on your CPU cores
pthread_t threads[num_threads];
ThreadData thread_data[num_threads];
 
// Split the workload
int chunk_size = natoms / num_threads;
for (int i = 0; i < num_threads; i++) {
    thread_data[i].start = i * chunk_size;
    thread_data[i].end = (i == num_threads - 1) ? natoms : (i + 1) * chunk_size;
    thread_data[i].coords = coords;
    thread_data[i].boxdims = boxdims;
    thread_data[i].atom_map = atom_map;
    thread_data[i].rcout_2b = rcout_2b;
    thread_data[i].rcout_3b = rcout_3b;
    thread_data[i].rcout_4b = rcout_4b;
    thread_data[i].cludists_2b_direct = cludists_2b_direct;
    thread_data[i].cludists_3b_direct = cludists_3b_direct;
    thread_data[i].cludists_4b_direct = cludists_4b_direct;
    thread_data[i].cludists_2b_trans = cludists_2b_trans;
    thread_data[i].cludists_3b_trans = cludists_3b_trans;
    thread_data[i].cludists_4b_trans = cludists_4b_trans;
